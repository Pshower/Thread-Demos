...
int chan_create(struct chan *ch) {
...
int chan_send(struct chan *ch, char c) {
    pthread_mutex_lock(&ch->lock);
    // 等待直到通道不为满
    while (ch->s[strlen(ch->s)] != ' ') {
        pthread_cond_wait(&ch->cond_empty, &ch->lock);
    }
    // 发送字符
    int len = strlen(ch->s);
    ch->s[len] = c;
    pthread_cond_signal(&ch->cond_fill); // 通知接收方数据已填充
    pthread_mutex_unlock(&ch->lock);
    return 0;
}
...
int chan_recv(struct chan *ch, char *c) {
    ...
    *c = ch->s[0];
    // 将剩余的字符前移
    memmove(ch->s, ch->s + 1, strlen(ch->s));
    ch->s[strlen(ch->s) + 1] = ' '; // 在末尾添加空格作为新的结束标记
    pthread_cond_signal(&ch->cond_empty); // 通知发送方通道已空出
    pthread_mutex_unlock(&ch->lock);
    return 0;
}
*/
